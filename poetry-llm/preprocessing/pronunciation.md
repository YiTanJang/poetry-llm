...
### 선정 및 정당화 근거
- **국제음성기호(IPA) 대비 이점**: IPA는 한국어 고유의 음운 규칙이나 자소 결합을 복잡하게 인코딩하며 토크나이저 Vocab에 큰 부담을 준다. 한글 발음 기호 표기는 기존 한국어 베이스 모델(Qwen2.5 등)의 기존 Vocab 및 임베딩 스페이스를 최대한 재활용할 수 있어 학습 효율이 극대화된다.
...
- 선택적 발음 어노테이션의 세부 임계값 설정 (어떤 음운 변동까지 명시적 태그로 남길 것인가? 예: 단순 연음화는 제외하고 경음화/격음화만 남길지 여부)
- 다국어(영어, 일본어 등) 발음 태깅이 한국어 토크나이저의 어휘 집합(Vocabulary) 팽창 및 학습 효율에 미치는 실제 영향 측정
- 추론(Generation) 시점에 모델이 스스로 생성하는 발음/음절 태그의 정합성을 실시간으로 강제(Guided Decoding)하기 위한 오버헤드 분석
