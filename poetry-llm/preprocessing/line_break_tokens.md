...
2. **공백 및 개행 바이트 인코딩 매핑**:
   - BPE/SentencePiece 기반 토크나이저(예: Gemma 4, Llama 계열)는 공백 문자(` `)를 바이트 단위(예: `_` 혹은 ` `)로 인코딩하며, 문장 내 단어와 특수 토큰이 결합될 때 불필요한 공백 토큰이 특수 토큰과 접합되어 학습되는 현상이 발생합니다.
...
4. **여백 크기 상한선 ($N_{max}$) 및 초과 공백 처리**:
   - 시각적 가독성 왜곡 방지를 위해 여백 토큰 `<여백:N>`의 $N$ 최대값을 40으로 제한합니다 ($N_{max} = 40$).
   - 전처리 단계에서 탐지된 공백 길이가 40을 초과할 경우, 초과분은 잘라내어(Truncation) `<여백:40>`으로 변환합니다.
...
if __name__ == "__main__":
    try:
        verify_special_tokens_integrity("google/gemma-4-27b")
...
- [User Review] 여백 크기의 최대값 임계치($N_{max}$) 결정: 여백 토큰 `<여백:N>`에서 `N`이 비정상적으로 큰 값(예: 80자 이상의 공백)으로 생성될 경우, 모델의 어휘 사전(Vocabulary)과 시각적 가독성에 심각한 왜곡이 생길 수 있다. 적절한 여백 제어 최대값 $N_{max}$는 몇으로 제한해야 하며, 그 이상의 공백은 어떻게 무력화하거나 처리해야 하는가? ($N_{max}=40$ 적용 수정 제안 추가함)
- [Ph1] 학습 시 행걸침 토큰 `<행갈이:걸침>`과 일반 행갈이 토큰 `<행갈이>`의 미학적 학습 기여도 평가: 두 종류의 행갈이 토큰이 미학적 독창성(Novelty) 향상에 기여하는 정도를 정량적으로 평가하기 위한 비교 실험(A/B 테스트 또는 토큰별 생성 확률 분석) 설계 방식과, 특정 시적 사조(예: 미래주의, 다다이즘)에 최적화된 행걸침 유도 프롬프트/파인튜닝 기법은 무엇인가?
