...
2. **시 전문 미세조정 평가 모델 (Expert Fine-tuned Evaluator)**:
   ...
   - **방안**: Gemma 4 27B/31B 또는 SOLAR-10.7B 모델에 한국 현대 평론집 약 50,000문장 및 전문가 채점 데이터셋을 파인튜닝하여 구축한다.
...
graph TD
    ...
    B -->|로컬 Gemma 4 9B / Swiss-system| C[Top 3 후보 선별]
    C --> D[2단계: 본선 토너먼트]
...
1.  **1단계: 예선 토너먼트 (Preliminary Round)**:
    - **대상**: $G=8$개 후보 전체.
    - **수행**: 경량 로컬 모델(Gemma 4 9B 또는 Gemma 4 27B/31B)을 심사위원으로 삼아 라운드 로빈(Round-Robin) 또는 스위스 시스템(Swiss-system) 토너먼트를 수행한다.
...
* **가문 채점 페널티 승수 ($\lambda_{\text{family}}$)**:
  ...
  * **예시**: Gemma 4 27B-IT 심사위원이 Gemma 4 27B/31B 생성 시를 평가할 때.
...
- [Ph1] 자가 선호 편향 페널티 승수($\lambda_{\text{self}} = 0.92$, $\lambda_{\text{family}} = 0.95$)는 고정 상수 값으로 적용할 때 최선인가, 아니면 판정 모델의 파라미터 크기(9B vs 27B)에 따라 동적으로 가중치를 부여해야 하는가?
