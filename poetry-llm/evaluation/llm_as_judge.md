...
LLM 심사위원이 지닌 고유한 인지적 편향을 억제하기 위한 제어 장치를 도입한다.

1. **자가 선호 편향 (Self-preference Bias) 제어**:
   - **문제**: LLM은 자신이 생성한 문장 스타일이나 디코딩 확률 분포와 일치하는 시를 더 높게 평가하는 경향이 있다 (arXiv 2510.15313).
   - **해결**: 블라인드 테스트 평가(Blind Evaluation) 프로토콜을 적용한다. 평가 입력에서 시를 생성한 모델의 식별자(ID) 및 관련 메타데이터를 완벽히 소거한다. 또한 채점을 담당하는 심사위원 모델의 사전학습 분포와 완전히 다른 계열의 모델(예: Qwen 생성 시 LLaMA Judge 사용)을 배심원단에 교차 배치한다.
2. **위치 및 길이 편향 (Position & Length Bias) 제어**:
...
- [ ] 인간 평론가와 LLM 심사위원단 간의 Weighted Kappa 수치를 실시간 계산하고 시각화하는 평가 분석 모듈 개발.
- [ ] 시 전문 미세조정 평가 모델(Evaluator)을 구축하기 위해 국문학 석박사급 연구자 채점 데이터 1,000건의 SFT 데이터셋 설계.
- [ ] 심사위원 토론 단계에서 발생하는 프롬프트 토큰 소모량 최적화를 위해 요약형 토론 프로토콜 가이드라인 작성.
