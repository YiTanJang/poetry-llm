...
2. **일반 한국어 리플레이 버퍼(Replay Buffer) 및 데이터 혼합 스케줄러(Data-Blend Scheduler)**:
   ...
   - **데이터 블렌딩 스케줄러(Data-Blend Schedulers)**:
     - **안정화 지향형 고정 스케줄(Static Blend Schedule)**: CPT 전 기간에 걸쳐 리플레이 버퍼 비율을 **3%**로 고정하여 일반 언어 모델링 성능을 일관되게 유지한다.
     - **망각 방지 앵커링 스케줄(Dynamic Anchored Decay Schedule)**: 초기 급격한 파라미터 편향을 막기 위해, CPT 초기 에포크에는 리플레이 비율을 **5%**로 높게 시작하여 일반 한국어 표상 영역을 고정(Anchoring)한 뒤, 학습 진행에 따라 점진적으로 **1%**까지 감쇄(Decay)시켜 후반부에는 시 도메인의 특화 문체와 리듬을 집중 학습하도록 한다.
     - **누적 망각 보상 스케줄(Stepwise Incremental Schedule)**: 학습 누적에 따른 성능 저하를 방지하기 위해 초기에는 리플레이 비율을 **1%**로 낮게 유지하여 특화 학습을 극대화하다가, 에포크 후반부로 갈수록 비율을 점진적으로 **5%**까지 인상하여 붕괴된 일반 어휘력을 회복시킨다.

   > 탐색중: 파일럿 단계(Phase 1)에서 고정형(3%)과 앵커링 감쇄형(5% → 1%) 스케줄러 간의 어휘력 보존 성능 및 시 생성 미학성(Novelty) 향상율을 비교하여 본 학습(Phase 2)에 적용할 최종 스케줄러를 확정할 예정이다.
...
- [Ph1] EXAONE 및 Qwen2.5 토크나이저의 어휘 사전에 시 특수 토큰(`<행갈이>`, `<연갈이>`) 추가 시, 임베딩 초기화 레이어의 손실 추이 모니터링 코드 구현.
- [TODO] 1%~5% Replay Buffer용 고품질 일반 한국어 말뭉치(뉴스, 나무위키, Wikipedia-KR 정제본 등)의 확보 및 데이터 블렌딩 스케줄러 연동 테스트.
...
- [TODO] 합성 데이터 상한 모순 해소: `data/augmentation.md`의 ≤30% 제한과 이 파일 section 5-2의 최대 45% 완화 제안이 충돌한다. [User Review] 의도적으로 CPT와 SFT 단계의 합성 데이터 비율 상한을 다르게 가져가는 것인지(예: SFT에서는 45%까지 허용하되 CPT에서는 30%로 제한), 아니면 프로젝트 전체적으로 단일 기준(30% 또는 45%)으로 통일해야 하는지 명확한 결정이 필요하다.
