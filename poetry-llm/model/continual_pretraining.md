...
| :--- | :---: | :---: | :--- |
| **한국 현대시** (공공도메인 & CC 라이선스) | **40.0%** | 120M | 한국어 시의 지배적인 현대적 행/연 구조, 이미지 연계 방식, 낯선 어휘 패턴 학습. |
...
| **일반 한국어 Replay Buffer** (위키, 뉴스 등) | **1.0%** | 3M | CPT 과정 중 일반 상식, 표준 문법 구조, 논리적 문맥 파악력을 보존하여 망각 현상 차단. |
...
6. **Replay Buffer (1% / 3M)**: Wikipedia-KR 정제 데이터와 고품질 뉴스 사설 등을 혼합하여 pre-training 지식의 유실을 방지하는 정규화(Regularization) 역할을 수행한다.
...
- DAPT 과정 중 일반 상식을 보존하기 위한 1% Replay Buffer의 고품질 한국어 말뭉치 데이터셋 최적의 수집 경로 및 검증 방안
- 추가되는 특수 토큰(`<행갈이>`, `<연갈이>`)의 초기 임베딩 벡터 학습이 다른 기존 임베딩의 표현 공간을 왜곡하지 않고 신속히 정렬되는지 여부 검증
- DAPT 학습 중 Perplexity의 하락이 최종 시 창작 단계에서의 미학적 독창성(Novelty) 및 다양성 지표 향상과 일치하는지에 대한 통계적 상관관계 연구
