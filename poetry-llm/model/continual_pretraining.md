...
| **합계** | **100.0%** | **300M** | **DAPT 통합 데이터 믹스 (Gemma 4 토크나이저 기준)** |
...
Gemma 4 27B/31B 모델의 풀 파인튜닝을 위해 설계된 DAPT 학습 하이퍼파라미터 및 하드웨어 연동 전략은 다음과 같다.
...
- **유효 배치 크기 (Effective Batch Size)**:
  ...
  - **토큰 수 기준**: $4,096 \text{ sequences} \times 4,096 \text{ (sequence length)} = 16,777,216 \text{ tokens/batch}$ (약 16.8M 토큰/배치)
