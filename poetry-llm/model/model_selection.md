...
CPT (Continual Pretraining) 및 DAPT 단계를 수행할 때, **사전 학습(Pre-trained) Base 모델**에서 출발할지, 이미 지시사항 튜닝 및 인간 정렬이 적용된 **Instruct/Chat 모델**에서 시작할지 결정해야 한다.

- **결정**: 본 프로젝트는 **사전 학습 Base 모델(Base Model)**에서 출발하는 것을 원칙으로 삼는다.
- **근거**:
  1. **Instruction Weights 손상 방지**: 이미 RLHF/DPO 등으로 정교하게 정렬된 Instruct 모델의 가중치는 CPT/DAPT 단계에서 대규모의 시 텍스트를 노출시켰을 때 심각하게 붕괴(drift)하며, 일반적인 지시 이행 능력(Instruction-following)이 완전히 상실된다.
  2. **스타일 바이어스 왜곡 차단**: Instruct 모델은 비평 설명문이나 일반 대화체를 출력하도록 고도로 조율되어 있어 시의 독창적인 파격(낯설게 하기, 문법 해체)을 기피하는 평탄화 편향이 학습 가중치에 이미 심어져 있다.
  3. **Replay Buffer의 유연성**: Base 모델에서 출발하므로 CPT 단계에서 Catastrophic Forgetting을 막기 위한 일반 대화형 데이터셋의 리플레이 비율을 대폭 낮추거나(1%~5% 수준) 단순 웹 텍스트로만 유지할 수 있어, 데이터셋 내 순수 시 텍스트의 가중치를 더 높일 수 있다.
...
- [User Review] Gemma 4의 라이선스 조건이 연구 목적 풀 파인튜닝과 향후 상업적 서비스 전환에 실제로 허용되는지 확인. 법무팀(legal)으로부터 Apache 2.0 라이선스 기준 검증(validation)을 받아야 한다. Apache 2.0이 아닌 경우 장기 활용 제약이 생긴다.
...
- [User Review] 음향/음운 표현이 내재화된 20-40B급 오픈 모델 등장 시 베이스 모델 재검토. 현재(2026 mid) 조건 충족 모델 없음 — Qwen2-Audio(7B), LLaMA-3-Omni(8B) 등은 스케일 부적합, Gemini/GPT-4o는 클로즈드. Hangul의 phonetic transparency로 인해 한국어 대형 모델이 텍스트 레벨의 암묵적 음운 지식을 이미 보유할 가능성도 고려. 현재는 CoT scratchpad 내 음운 계획 분리 방식이 현실적 대안.
