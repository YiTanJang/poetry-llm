...
- [TODO] 음향/음운 표현이 내재화된 20-40B급 오픈 모델 등장 시 베이스 모델 재검토. 현재(2026 mid) 조건 충족 모델 없음 — Qwen2-Audio(7B), LLaMA-3-Omni(8B) 등은 스케일 부적합, Gemini/GPT-4o는 클로즈드. Hangul의 phonetic transparency로 인해 한국어 대형 모델이 텍스트 레벨의 암묵적 음운 지식을 이미 보유할 가능성도 고려. 현재는 CoT scratchpad 내 음운 계획 분리 방식이 현실적 대안.
