...
type: Design
title: 베이스 모델 선정
description: 후보 모델(Gemma 4, Qwen2.5, LLaMA-3 등)의 토크나이저 효율성, 크기, 라이선스 비교 분석 — 베이스 모델 미확정, 검토 진행 중.
tags: [model, selection, llm, gemma, qwen, llama, solar, infrastructure]
...
[Phase 3: 비교 실험 (A/B Testing)]
  - 한국어 특화 모델(예: EXAONE-3.5-32B)과의 시 품질 정성 평가 비교 진행
  - 토크나이저 효율성에 따른 학습 속도 및 문학적 다양성(Novelty) 교차 검증
```
