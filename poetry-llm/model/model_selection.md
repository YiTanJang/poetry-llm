...
type: Design
title: 베이스 모델 선정
description: Qwen2.5-32B, LLaMA-3-70B, SOLAR-10.7B 모델의 토크나이저 효율성, 크기, 라이선스 제약 조건 비교 분석.
tags: [model, selection, llm, qwen, llama, solar, infrastructure]
...
## Qwen2.5-32B vs LLaMA-3-70B vs SOLAR-10.7B 심층 비교

프로젝트의 베이스 모델 선정을 위해 후보군인 세 모델을 세 가지 핵심 차원(토크나이저 효율성, 모델 크기, 라이선스 제약)과 임베딩 확장성 관점에서 비교 분석한다.
...
graph TD
    ...
    C --> C1["크기: 70.6B (인프라 비용 과다)"]
    C --> C2["토크나이저: 128,256 (보통의 한국어 효율)"]
    C --> C3["라이선스: LLaMA 3 License (조건부 상업용)"]
    
    D --> D1["크기: 10.7B (파일럿에 적합한 초경량)"]
    D --> D2["토크나이저: 32,000 (한국어 분절 심함)"]
    D --> D3["라이선스: Apache 2.0 (상업적 이용 제한 없음)"]
```
...
[Phase 3: 비교 실험 (A/B Testing)]
  - LLaMA-3-70B 또는 한국어 특화 모델(예: EXAONE-3.5-32B)과의 시 품질 정성 평가 비교 진행
  - 토크나이저 효율성에 따른 학습 속도 및 최종 생성된 시의 문학적 다양성(Novelty) 교차 검증
```
...
- SOLAR-10.7B(32K vocab)의 낮은 한국어 토큰 효율이 긴 창작 노트를 포함하는 포맷에서 실질적으로 모델의 유효 컨텍스트(Context Window)를 얼마나 제한하는지, 그리고 이를 보완하기 위해 Ko-Solar 등 커뮤니티 어휘 확장 버전을 도입할 때 발생하는 파인튜닝 불안정성 해결 방안은 무엇인가?
- LLaMA-3-70B가 보여주는 독보적인 은유 및 시적 표현 능력을 활용하기 위해, 풀 파인튜닝 대신 QLoRA나 DeepSpeed CPU Offloading을 적용했을 때의 학습 속도 및 수렴 품질이 Qwen2.5-32B 풀 파인튜닝 대비 경쟁력이 있는지 검증할 실험 설계 방법은?
- Qwen2.5-32B가 한자(Hanja) 및 동양 고전 시의 형식미를 다국어 임베딩 공간에서 전이받을 때, 현대 한국어 자유시에서 지나치게 고풍스럽거나 어색한 번역투 문체가 지배적이 되는 문제를 방지하기 위해 SFT 단계에서 현대 한국어 구어/문학 코퍼스의 비율을 어떻게 동적으로 설계할 것인가?
