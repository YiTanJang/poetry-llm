...
type: Design
title: 파인튜닝 전략
description: 전체 학습 파이프라인 — CPT(도메인 적응) → SFT(창작 노트+시 포맷) → GRPO 또는 DPO(선호 정렬). GRPO를 우선 권장(Reference model 불필요, VRAM 절감, 그룹 상대 보상). 풀 파인튜닝 vs LoRA 결정 근거, 하이퍼파라미터 전략 포함.
tags: [model, fine-tuning, full-finetuning, lora, training, dpo, grpo, cpt, sft]
...
SFT (Supervised Fine-Tuning)
  └── 창작 노트(CoT) → 시 포맷 학습, 특수 토큰 통합
        ↓
선호 정렬
  ├── GRPO (권장) — 그룹 샘플링 + 상대 보상, Reference model 불필요
  └── DPO  (대안) — (winner, loser) 쌍 기반 오프라인 정렬
```

각 단계는 순차적이다. CPT 없이 SFT를 하면 도메인 언어 적응이 부족하고, SFT 없이 선호 정렬을 하면 기본 포맷 자체가 불안정하다.
...
## 3단계: 선호 정렬 — GRPO (권장) / DPO (대안)

### 목적

SFT로 학습한 포맷 위에 미학적 선호도를 정렬한다. "어떤 시가 더 나은가"라는 판단을 학습 신호로 내재화한다.

---

### GRPO (Group Relative Policy Optimization) — 권장

**선택 이유:**

| 항목 | DPO | GRPO |
|------|-----|------|
| Reference model | 필요 (VRAM 2배) | 불필요 |
| 보상 신호 | binary (chosen/rejected) | 연속 점수 + 그룹 상대화 |
| 시 도메인 적합성 | 미학 품질이 이분법으로 표현됨 | 연속 미학 점수에 자연스럽게 매핑 |
| Likelihood drift | 발생 가능 | 없음 |
| 주요 사용처 | 범용 정렬 | DeepSeek-R1, Qwen2.5-Math, 주관적 창의 태스크 |

**3 페르소나 설계와의 연결:**

현재 페르소나 A/B/C가 각각 1편을 생성하는 구조 → GRPO의 G개 그룹 생성으로 자연스럽게 확장 가능. 같은 창작 노트 프롬프트로 페르소나들이 복수 편을 생성하면 그룹이 구성된다.

**알고리즘:**

```
1. 동일 창작 노트 프롬프트로 G=8 편 병렬 생성
2. 외부 judge 또는 리워드 모델이 각 출력에 점수 부여: s₁, s₂, ..., s₈
3. 그룹 상대 보상 계산: r_i = (s_i − mean(s)) / std(s)
4. Reference model 없이 정책 업데이트 (KL 패널티 낮게 또는 0)
```
...
{
  "prompt": "창작 노트 시드 텍스트",
  "outputs": ["시 1", "시 2", "시 3", "시 4", "시 5", "시 6", "시 7", "시 8"],
  "scores": [0.82, 0.74, 0.68, 0.61, 0.55, 0.50, 0.44, 0.38],
  "judge_model": "claude-opus-4-8",
  "timestamp": "2026-06-28T00:00:00Z"
}
```

저장 경로: `data/grpo_groups/`

**GRPO 하이퍼파라미터:**

> 탐색중 — G, β, 학습률은 Phase 2 실험으로 결정됨

| 파라미터 | 초기값 | 비고 |
|----------|--------|------|
| G (group size) | 8 | 한 프롬프트당 생성 편 수 |
| Learning rate | 5e-7 | SFT보다 낮게 |
| β (KL penalty) | 0.0~0.01 | Reference model 없으므로 낮게 유지 |
| Score normalization | group mean/std | 그룹 내 상대화 |
| Epochs | 1~2 | 과적합 주의 |

---

### DPO (Direct Preference Optimization) — 오프라인 대안

GRPO보다 구현이 단순하나 두 가지 제약이 있다: (1) Reference model을 유지해야 해 VRAM 부담이 크고, (2) 미학 품질을 binary로 표현하면 미세한 품질 차이가 손실된다. Phase 1 파일럿에서 구현 단순성이 중요할 때 사용.

**선호 쌍 구성 (3 페르소나 방식):**

```
1. 동일한 프롬프트로 3개 시인 페르소나가 각각 초안 생성:
   - 페르소나 A: 이미지 중심 (황지우 계열 — 낯선 이미지 연결, 시각적 충격)
   - 페르소나 B: 리듬·음악성 중심 (김혜순 계열 — 반복, 음운, 몸의 언어)
   - 페르소나 C: 관념·해체 중심 (이상 계열 — 논리 해체, 개념 충돌)

2. 외부 심사 모델이 3개 초안을 비교, 6개 미학 기준으로 평가
3. 최고 점수 초안 → chosen / 최저 점수 초안 → rejected
...
정렬 이후 생성 샘플을 평가 파이프라인([evaluation_pipeline.md](evaluation_pipeline.md))에 넣어 리워드 모델 점수와 인간 평가 점수 변화를 추적한다. 선호 쌍의 margin이 작으면 (score_chosen − score_rejected < 0.2) weak pair로 제거하거나 GRPO 전환 검토.
...
- [Ph2] **GRPO vs DPO 비교 실험**: G=8 그룹 GRPO와 (winner, loser) DPO를 동일 SFT 체크포인트에서 출발하여 비교. 미학적 선호도 점수 향상, Novelty 지표 변화, VRAM 사용량 차이 측정.
- [Ph2] **GRPO group size G 최적값**: G=4 vs G=8 vs G=16 — 생성 다양성, 점수 신호 분산, 학습 안정성 트레이드오프. 외부 judge 호출 비용도 G에 비례하므로 경제성도 고려.
- [Ph2] **GRPO β 값 및 SFT 체크포인트 선정**: 선호 정렬 시작 지점(Stage 3 vs Stage 4 종료 시점)과 β 범위(0.0~0.05)가 수렴 속도와 Novelty에 미치는 영향.
...
- [Ph2] **On-Policy Self-Distillation 도입 가능성**: SPIN 또는 Self-Rewarding LM 방식을 시 도메인에 적용 시, 외부 judge 의존 GRPO/DPO 대비 생성 novelty 및 미학적 선호도 점수 변화. 자기 선호 편향 관리 방안 포함.
- [TODO] **다국어 시 데이터의 어휘사전 확장과 토큰 파편화 방지**: 외국어 시 번역 및 학습 시, 한국어와 외국어 토큰 파편화(fragmentation)를 최소화하면서 시적 운율과 뉘앙스를 보존할 수 있는 최적의 Vocabulary 크기 및 추가 토큰 확장 기법은 무엇인가? 특히 토큰화 방식이 다국어 시의 원어적 뉘앙스와 각운(rhyme), 리듬감 등 미학적 패턴에 미치는 영향은 어떻게 평가하고 보완할 것인가?
- [TODO] **G=8 GRPO 그룹과 3 페르소나 카운트 불일치 해소**: 3 페르소나 × 1편 = 3편이지만 GRPO는 G=8 그룹이 필요하다. 같은 페르소나로 다른 temperature에서 재샘플링(3×3 구성), 페르소나를 8개로 늘리거나, 역추론 데이터를 섞어 G=8을 채우는 방안 중 어느 것이 다양성과 비용 면에서 최선인가? (Q31 참조)
- [Ph2] **GRPO 반복 학습에서 페르소나 모드 붕괴 방지**: 일관적으로 높은 보상을 받는 한 페르소나 스타일로 전체 출력이 수렴(모드 붕괴)할 수 있다. 보상 함수에 페르소나 간 분산 보너스(inter-persona variance reward)를 추가하는 방안 실험. 다양성 보너스의 강도와 품질 보상의 트레이드오프 측정 필요. (Q34 참조)
