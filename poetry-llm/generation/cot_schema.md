...
- 스크래치패드의 최적 길이 분포: 너무 짧으면 탐색이 없고, 너무 길면 추론 비용이 증가한다. 실험으로 결정해야 한다.
- 씨앗과 금지 앵커를 완전히 제거하면 어떻게 되는가 — 완전 자유형 대비 ablation 필요.
- 스크래치패드 형식이 학습 후 어떻게 수렴하는가: 모델이 선호하는 내부 패턴이 나타나는가, 아니면 계속 다양한가.
- 프로세스 감독 신호를 스크래치패드의 어느 단계에 줄 것인가 — 중간 보상 설계는 별도 연구 필요.
- 스크래치패드가 너무 길어지면 시 자체의 생성이 압축되는 현상이 있는가.
