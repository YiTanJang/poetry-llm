...
| 항목 | 결정 |
|------|------|
| 베이스 모델 | Qwen2.5-32B (파일럿: 10.7B) |
| 파인튜닝 | CPT → SFT → DPO (풀 파인튜닝, LoRA는 파일럿만) |
| 특수 토큰 | `<행갈이>`, `<연갈이>`, `<시작>`, `<끝>` |
| 생성 파이프라인 | 자유형 CoT 스크래치패드 → 3 시인 페르소나 → 외부 심사 → DPO |
| 자동 필터 | 최소 sanity gate만 (공백 여부, 반복 80%, 3-100행) |
| 평가 | 0.50×인간 + 0.30×리워드 모델 + 0.20×LLM 심사 |
